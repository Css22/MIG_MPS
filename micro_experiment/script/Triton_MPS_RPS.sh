...
for percentage in "${percentage_list[@]}"; do
    echo set_active_thread_percentage 2741354 $percentage| nvidia-cuda-mps-control
    ...
    (sleep 120 && cd /data/zbw/inference_system/MIG_MPS/inference_system && /home/zbw/anaconda3/envs/Abacus/bin/python client.py --task resnet50 --config $percentage && (docker stop $(docker ps -q)))
    sleep 60
done
